...
-- https://gist.github.com/MCJack123/42bc69d3757226c966da752df80437dc
...
for key, value in pairs(extra_allowed_apis) do
    keptAPIs[key] = value
end
...
function _G.term.native()
    ...
    term.clear()
    local file = fs.open("/frekos/boot/bios.lua", "r")
    if file == nil then
        term.setCursorBlink(false)
        term.setTextColor(16384)
        term.write("Could not find /frekos/boot/bios.lua. UnBIOS cannot continue.")
    ...
    if fn == nil then
        term.setCursorBlink(false)
        term.setTextColor(16384)
        term.write("Could not load /frekos/boot/bios.lua. UnBIOS cannot continue.")
        term.setCursorPos(1, 2)
        term.write(err)
